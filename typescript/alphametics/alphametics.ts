export function solve(puzzle: string): Record<string, number> | undefined {
  ...
  // Backtracking
  function search(idx: number, used: boolean[], mapping: Record<string, number>): Record<string, number> | undefined {
    if (idx === letters.length) {
      ...
      const leftSum = leftWords.reduce((sum, w) => sum + wordValue(w, mapping), 0);
